...
function buildPayload(debrief, history){
  ...
  return {
    ...
    me: {
      // the player's own name (3/9): the persona used to carry the author's
      // name as a constant, so every other player was addressed as him
      name: (debrief.coachee && debrief.coachee.name) || null,
...
const FORM_RULE =
  'Spillerens navn står i me.name (null = ukendt; brug så ikke noget navn). '
...
const FORM_RULE_EN =
  'The player\'s name is in me.name (null = unknown; then use no name). '
