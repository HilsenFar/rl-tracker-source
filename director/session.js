...
/* No built-in player (3/9): the id used to be the author's, so every copy of
 * the product started out reading the author's rows. The tracked player comes
 * from profile.json at init, from rehome() in guest mode, and — per match —
 * from the debrief's own coachee field (director.onDigest stamps it). */
const DEFAULT_PID = '';
...
function onMatch(ev){
  ...
  // whose match this is: the debrief says (director stamps coachee on every
  // card), and the stored pid covers a debrief written before that field
  // existed. Without either there is nobody to report on.
  ...
  if (c.metricId){
    ...
    reason = en
      ? 'Variety, same measured need as tonight — ' + t.label + ' ' + M.fmt(t.id, t.sessionAvg)
        + ' against your normal ' + M.fmt(t.id, t.baselineAvg) + ' in ' + t.playlist + ': ' + (p.whatEn || p.what) + '.'
    ...
    reason = en
      ? 'Variety, same measured need as tonight — ' + earlyConceded + ' goal' + (earlyConceded > 1 ? 's' : '')
        + ' conceded in the first minute of a match: ' + (p.whatEn || p.what) + '.'
    ...
    reason = en
      ? 'Variety for your own aerial goal — no measurement behind it (the feed has no air data): ' + (p.whatEn || p.what) + '.'
      : 'Afveksling til dit eget aerial-mål — ingen måling bag (feedet har ingen luftdata): ' + p.what + '.';
  } else {
    reason = en
      ? 'Variety, no measured coupling tonight: ' + (p.whatEn || p.what) + '.'
...
function renderHTML(r){
  const en = EN();   // rammen foelger motorens sprog (26/8) — som weekly
  const durText = r.durationMin >= 60
    ? Math.floor(r.durationMin / 60) + (en ? ' h ' : ' t ') + (r.durationMin % 60) + ' min'
    : r.durationMin + ' min';

  const stopBanner = r.tilt.stop
    ? '<div class="card stop"><strong>' + (en ? 'Tilt guard' : 'Tilt-vagt') + '</strong><p>' + esc(r.tilt.stopText) + '</p></div>'
    : '';
  ...
  const plCards = Object.keys(r.playlists).map(k => {
    ...
    return '<div class="card"><div class="plhead"><strong>' + esc(k) + '</strong>' +
      ...
      '<span class="chip">' + pl.goals + (en ? ' goals / ' : ' mål / ') + pl.shots + (en ? ' shots' : ' skud') +
      (pl.conversion !== null ? ' · ' + Math.round(pl.conversion * 100) + (en ? '% conversion' : '% konvertering') : '') + '</span></div>' +
      '<div class="scroll"><table><thead><tr>' + (en
        ? '<th>Start</th><th>Res.</th><th>Score</th><th>Goals</th><th>Ast.</th><th>Saves</th><th>Shots</th><th>Touch</th>'
        : '<th>Start</th><th>Res.</th><th>Score</th><th>Mål</th><th>Ass.</th><th>Redn.</th><th>Skud</th><th>Touch</th>') + '</tr></thead>' +
  ...
  const privRows = (r.privateMatches || []).map(m =>
    '<tr><td>' + hhmm(m.startedAt) + '</td><td>' + esc(m.playlist || '–') + ' <span class="tag">' + (en ? 'private' : 'privat') + '</span></td>' +
    '<td>' + esc(m.matchType || (en ? 'private' : 'privat')) + '</td>' +
    '<td class="' + (m.result === 'W' ? 'W' : m.result === 'L' ? 'L' : 'muted') + '">' + (m.result || '–') + '</td>' +
    '<td class="num">' + myScore(m) + '</td>' +
  ...
  const privCard = privRows
    ? '<div class="card"><div class="plhead"><strong>' + (en ? 'Private lobbies' : 'Private lobbyer') + '</strong><span class="chip">' + (en ? 'shown, not counted' : 'vist, ikke talt') + '</span></div>' +
      ...
      '<p class="muted" style="font-size:12.5px">' + (en
        ? 'A private lobby is whoever the host invited, at whatever level the host chose — it enters neither W/L, trends, missions nor your normal.'
        : 'En privat lobby er hvem værten inviterede, på det niveau værten valgte — den indgår hverken i W/L, trends, missioner eller din normal.') + '</p></div>'
    : '';
  ...
  }).join('') : '<tr><td colspan="6" class="muted">' + (en ? 'No metrics with data in this session.' : 'Ingen metrikker med data i denne session.') + '</td></tr>';

  const seqChips = r.tilt.sequence.split(' ').map(x =>
    '<span class="seq ' + (x === 'W' ? 'W' : x === 'L' ? 'L' : '') + '">' + esc(x) + '</span>').join('');
  const driftHtml = r.tilt.drift
    ? (en
      ? 'Touch power average, first half of the matches: <strong>' + M.fmt('hit_power_avg', r.tilt.drift.firstAvg) +
        '</strong> · second half: <strong>' + M.fmt('hit_power_avg', r.tilt.drift.secondAvg) +
      ...
      : 'Touchkraft-snit, kampene i første halvdel: <strong>' + M.fmt('hit_power_avg', r.tilt.drift.firstAvg) +
        ...
        '</strong> (drift <strong>' + pctS(r.tilt.drift.pct) + '</strong>).')
    : (en ? 'Touch power drift: too few measurements to compare the session’s halves.'
  ...
  const notesHtml = r.notes.length ? r.notes.map(n =>
    ...
    (n.source ? '<p class="src">' + (en ? 'Source: ' : 'Kilde: ') + '<a href="' + esc(n.source.url) + '" rel="noopener">' + esc(n.source.title) + '</a></p>' : '') + '</div>').join('')
    : '<p class="muted">' + (en
      ? 'No strategy notes tonight — no sourced rule matches the session’s weak points.'
      : 'Ingen strateginoter i aften — ingen kildebelagt regel matcher sessionens svage punkter.') + '</p>';

  return reportTheme.htmlOpen((en ? 'en' : 'da')) + '<head><meta charset="utf-8">\n' +
    reportTheme.BOOT + '\n' +
    '<meta name="viewport" content="width=device-width,initial-scale=1">\n' +
    '<title>' + (en ? 'Session report — ' : 'Session-rapport — ') + esc(danishDate(r.startedAt, en)) + '</title>\n' +
    '<style>\n' +
    /* NITRO (27/8) — samme plade som ugerapporten og trackeren. Selvstaendig
     * fil: ingen billed-afhaengighed, og fonten er progressiv (findes naar
     * rapporten laeses fra /reports/, falder ellers tilbage til Segoe UI). */
    ...
    '.stop{border-color:rgba(255,201,74,.55);background:linear-gradient(163deg,#2A2110 0%,#1C1608 100%)}\n' +
    ...
    '<h1>' + (en ? 'Tonight’s session' : 'Aftenens session') + '</h1>\n' +
    '<div class="sub">' + esc(danishDate(r.startedAt, en)) + ' · ' + hhmm(r.startedAt) + '–' + hhmm(r.endedAt) + '</div>\n' +
    '<div>' +
      '<span class="chip">' + r.matches.length + (en ? ' matches' : ' kampe') + '</span>' +
    ...
    '<p class="muted" style="font-size:12.5px">' + (en
      ? 'Normal = your own running EWMA average per playlist (profile.json) — no external benchmarks.'
      : 'Normal = dit eget løbende EWMA-snit pr. playlist (profile.json) — ikke eksterne benchmarks.') + '</p></div>\n' +
    '<h2>' + (en ? 'Tilt analysis (measured patterns only)' : 'Tilt-analyse (kun målte mønstre)') + '</h2>\n' +
    '<div class="card">' +
    '<div>' + seqChips + '</div>' +
    '<ul class="tilt">' +
    '<li>' + driftHtml + '</li>' +
    '<li>' + (en ? 'Goals conceded in the first minute of a match: ' : 'Mål indkasseret i kampens første minut: ') + '<strong>' + r.tilt.earlyConceded + '</strong></li>' +
    '<li>' + (en ? 'The team’s goals in the final minute or overtime: ' : 'Holdets scoringer i sidste minut eller overtid: ') + '<strong>' + r.tilt.lateOwn + '</strong></li>' +
    '</ul>' +
    ...
    '<footer>' + (en
      ...
      : 'Genereret ' + esc(danishDate(r.at)) + ' kl. ' + hhmm(r.at) + ' · session-rapport (' + esc(r.reason) + ') · alle tal er målt af trackeren — ingen opfundne benchmarks.') + '</footer>\n' +
    '</div></body></html>\n';
}
