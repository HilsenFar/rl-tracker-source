@echo off
rem Opens the always-on-top overlay (single-screen play).
...
if exist "%~dp0..\RLOverlay.exe" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\launch-overlay.ps1" %*
  start "" /min powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0..\launch-overlay.ps1" -Watch
)
